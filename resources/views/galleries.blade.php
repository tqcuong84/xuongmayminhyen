        ...
        <div class="row">
            <div class="col col-xs-12 sortable-gallery">
                <div class="gallery-container gallery-fancybox masonry-gallery">
                    ...
                    <div class="grid" id="gallery-{{ $gallery->id }}">
        ...
        timeout_load_more = setTimeout(function(){
            $.ajax({
                ...
                success: function(response) {
                    loading = false;
                    elm_loading.hide();
                    if(response.galleries !== undefined && response.galleries !== null && response.galleries.data !== undefined){
                        let _html = "";
                        let has_change = false;
                        response.galleries.data.forEach(item => {
                            if($("#gallery-" + item.id).length == 0){
                                _html += '<div class="grid" id="gallery-'+item.id+'">' + 
                                    '<a href="'+item.avatar_file+'" class="fancybox" data-fancybox-group="gall-1">' + 
                                    '<img src="'+item.avatar_file+'" alt="'+item.title+'" class="img img-responsive" />' + 
                                    '</a></div>';
                                has_change = true;
                            }
                        });
